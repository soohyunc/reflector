...
void usage()
{
        printf("reflector [-f] [-n] [-c <port>] [-k <known host1>,<known host2>,..] port1 port2 ...\nwhere:\n\t -f causes packets to be forwarded rather than reflected.\n\t -n means no user interface.\n\t -c <port> accept control commands on the given port.\n");
}
...
process_cmd(int sock_fd, struct queue_s* channels)
{
        ...
        while (nbytes > 0) {
                ...
                case ID_PORT:
                        ...
                        chan = NULL;
                        tgt_port = atoi(sport);
                        for(i = queue_len(channels)-1; i >= 0; i--) {
                                chan = (channel_t*) get_item_no(channels, i, Q_KEEP);
                     ...
                     struct queue_s* channels)
{
        ...
        while( (s = accept(control_fd, &incoming, &len)) != -1) {
                ...
                *new_sock     = s;
                add_to_queue(active_socks, (char*)new_sock);
                printf("New connection accepted.\n"); 
/*                if (setsockopt(*new_sock, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(char)) != 0) {
        ...
        nconn = queue_len(active_socks);
        FD_ZERO(&strm_set);
        for(i = 0; i < nconn; i++) {
                s = *((unsigned int*)get_item_no(active_socks, i, Q_KEEP));
                FD_SET(s, &strm_set);
        }
...
add_if_unknown(channel_t *cp, struct sockaddr_in *from)
{
        ...
        assert(cp->hosts);
        i = (unsigned int*)get_matching(cp->hosts,(char*)&a, Q_KEEP);
        if (!i) {
                i = (unsigned int*)malloc(sizeof(unsigned int));
                *i = a;
                add_to_queue(cp->hosts,(char*)i);
        }
        return;
}
...
queue_pkt(channel_t *cp, struct queue_s* pkts) {
        ...
        p->data     = (char*)malloc(MAX_PKT_SZ); /* give or take a few bytes */
        p->channel  = cp;
        p->arr_time = get_ms();
...
send_pkts(struct queue_s *pkts)
{
        ...
        now = get_ms();
        while((p=(pkt_t*)get_less_than(pkts,(char*)&now,Q_DETACH))) {
                assert(now > p->go_time);
                add_item_to_refl_stat(p->channel->err_dist, 
                                      now - p->go_time);
    
                if (drand48()>p->channel->loss) {
                        p->channel->forw++;
                        n = queue_len(p->channel->hosts);
                        for(i=0;i<n;i++) {
                                addr = (unsigned int*)get_item_no(p->channel->hosts,i,Q_KEEP);
...
main(int argc, char *argv[])
{
        ...
        channels      = create_queue(cmp,0);
        pkts          = create_queue(cmp,0);
        control_socks = create_queue(cmp,0);
        ...
        while(ac<argc) {
                ...
                tc->hosts = create_queue(cmp,0);
                tc->port  = atoi(strtok(argv[ac],","));
                tc->err_dist = new_refl_stat();
                if ((tok = strtok(NULL,","))) 
                ...
                add_to_queue(channels,(char*)tc);
                ac++;
        }

        if (queue_len(channels)) {
                init_reflect_sockets(channels);
                if (known_hosts) add_hosts(channels, known_hosts);
                if (ui_on) {
                        channels_to_ui(channels); /* order of these two fn's counts*/
                        init_ui(channels);
                }
                while(loop) {
        ...
        i = queue_len(channels);
        while(i-- > 0) {
                tc = (channel_t*)get_item_no(channels, 0, Q_DETACH);
                printf("Stats for port %d:\nDropped %d / %d \nDelay Distn\n",
                       tc->port,
                       tc->drop,
                       tc->forw);
                dmp_refl_stat(tc->err_dist);
                free_refl_stat(tc->err_dist);
                free(tc);
        }
